...
- Model catalog cache entries are fresh for 5 minutes. Fresh caches avoid repeated provider list requests, but cached rows are filtered to sources with currently configured API keys so stale multi-provider caches do not surface inaccessible models. Stale caches trigger provider refresh and remain available only as a fallback when refresh fails.
