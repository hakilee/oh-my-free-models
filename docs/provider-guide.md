...
- Provider model catalogs are cached for 5 minutes and filtered to providers with currently configured API keys before reuse; stale catalogs are refreshed before normal use, with stale-cache fallback only when provider catalog fetches fail.
